...
func _physics_process(_delta: float) -> void:
	...
	_pending_flags = 0
	var should_stop := (_stop_tick >= 0 and tick >= _stop_tick) or frames.size() >= MAX_TICKS
	if should_stop:
		if frames.size() >= MAX_TICKS and _stop_tick < 0:
...
func _print_results() -> void:
	...
	var dnf_count := marbles.size() - placements.size()
	if dnf_count > 0:
